...
export function SetNewComboItemsFix2(combo, items) {
    if (!combo) return;
    ...
    if (typeof items === "object" && (typeof items.text[0] === "string" && typeof items.text !== "string")) {
        for (let i = 0; i < items.text.length; i++) {
            AddNewComboItem(combo, items.value[i], items.text[i]);
        }
    } else {
...
export function ToggleComponentCl(element, className) {
    if (!element) return;
