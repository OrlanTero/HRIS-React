...
export function SelectBank() {
    const popup = new Popup("banks/select_bank", null, {
        backgroundDismiss: false,
    });

    return new Promise((resolve) => {
        const SelectRequest = (id) => {
            GetBank(id).then((bank) => resolve(bank)).finally(() => {
                popup.Remove();
            });
        }

        function Search(toSearch, filter) {
            SearchRecords("banks", toSearch, filter).then((HTML) => UpdateTable(HTML));
        }
...
export function SelectLoans(id) {
    ...
    return new Promise((resolve) => {
        ...
        function Search(toSearch, filter) {
            SearchRecords("employment", toSearch, filter).then((HTML) => UpdateTable(HTML));
        }
...
export function RestoreArchive(ids) {
    return new Promise((resolve) => {
        Ajax({
            url: "/api/post/RestoreArchive",
            type: "POST",
            data: {ids: JSON.stringify(ids)},
            success: (res) => {
                try {
