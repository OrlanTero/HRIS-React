            ...
            $this->KLEIN->with("/admin", function () use ($KLEIN, $APPLICATION) {
                ...
                $KLEIN->with("/employees", function () use ($KLEIN, $APPLICATION) {
                    ...
                    $KLEIN->respond("POST", "/removeRecords", function () use ($APPLICATION) {
                        return json_encode($APPLICATION->FUNCTIONS->EMPLOYEE_CONTROL->archiveRecordsWithLog(json_decode($_POST["data"], true)));
                    });
                });
                ...
                $KLEIN->with("/mortuary", function () use ($KLEIN, $APPLICATION) {
                    ...
                    $KLEIN->respond("POST", "/removeRecords", function () use ($APPLICATION) {
                        return json_encode($APPLICATION->FUNCTIONS->MORTUARY_CONTROL->archiveRecordsWithLog(json_decode($_POST["data"], true)));
                    });
                });
                $KLEIN->with("/requisition", function () use ($KLEIN, $APPLICATION) {
                    ...
                    $KLEIN->respond("POST", "/removeRecords", function () use ($APPLICATION) {
                        return json_encode($APPLICATION->FUNCTIONS->REQUISITION_CONTROL->archiveRecordsWithLog(json_decode($_POST["data"], true)));
                    });
                });
                ...
                $KLEIN->with("/employment", function () use ($KLEIN, $APPLICATION) {
                    ...
                    $KLEIN->respond("POST", "/removeRecords", function () use ($APPLICATION) {
                        return json_encode($APPLICATION->FUNCTIONS->EMPLOYMENT_CONTROL->archiveRecordsWithLog(json_decode($_POST["data"], true)));
                    });
                });
                ...
                $KLEIN->with("/clients", function () use ($KLEIN, $APPLICATION) {
                    ...
                    $KLEIN->respond("POST", "/removeRecords", function () use ($APPLICATION) {
                        return json_encode($APPLICATION->FUNCTIONS->CLIENT_CONTROL->archiveRecordsWithLog(json_decode($_POST["data"], true)));
                    });
                });
                ...
                $KLEIN->with("/banks", function () use ($KLEIN, $APPLICATION) {
                    ...
                    $KLEIN->respond("POST", "/removeRecords", function () use ($APPLICATION) {
                        return json_encode($APPLICATION->FUNCTIONS->BANK_CONTROL->archiveRecordsWithLog(json_decode($_POST["data"], true)));
                    });
                });
                ...
                $KLEIN->with("/adjustment", function () use ($KLEIN, $APPLICATION) {
                    ...
                    $KLEIN->respond("POST", "/removeRecords", function () use ($APPLICATION) {
                        return json_encode($APPLICATION->FUNCTIONS->ADJUSTMENT_CONTROL->archiveRecordsWithLog(json_decode($_POST["data"], true)));
                    });
                });
                $KLEIN->with("/pettycash", function () use ($KLEIN, $APPLICATION) {
                    ...
                    $KLEIN->respond("POST", "/removeRecords", function () use ($APPLICATION) {
                        return json_encode($APPLICATION->FUNCTIONS->PETTYCASH_CONTROL->archiveRecordsWithLog(json_decode($_POST["data"], true)));
                    });
                });
                $KLEIN->with("/holidays", function () use ($KLEIN, $APPLICATION) {
                    ...
                    $KLEIN->respond("POST", "/removeRecords", function () use ($APPLICATION) {
                        return json_encode($APPLICATION->FUNCTIONS->HOLIDAY_CONTROL->archiveRecordsWithLog(json_decode($_POST["data"], true)));
                    });
                });

                $KLEIN->with("/client_holidays", function () use ($KLEIN, $APPLICATION) {
                    $KLEIN->respond("POST", "/addRecord", function () use ($APPLICATION) {
                        return json_encode($APPLICATION->FUNCTIONS->CLIENT_HOLIDAY_CONTROL->addRecordWithLog(json_decode($_POST["data"], true)));
                    });

                    $KLEIN->respond("POST", "/editRecord", function () use ($APPLICATION) {
                        return json_encode($APPLICATION->FUNCTIONS->CLIENT_HOLIDAY_CONTROL->editRecordWithLog($_POST["id"], json_decode($_POST["data"], true)));
                    });

                    $KLEIN->respond("POST", "/removeRecords", function () use ($APPLICATION) {
                        return json_encode($APPLICATION->FUNCTIONS->CLIENT_HOLIDAY_CONTROL->archiveRecordsWithLog(json_decode($_POST["data"], true)));
                    });
                });
                ...
                $KLEIN->with("/system_types", function () use ($KLEIN, $APPLICATION) {
                    ...
                    $KLEIN->respond("POST", "/editRecord", function () use ($APPLICATION) {
                        return json_encode($APPLICATION->FUNCTIONS->SYSTEM_TYPES_CONTROL->editRecordWithLog($_POST['id'], json_decode($_POST["data"], true)));
                    });
                ...
                $KLEIN->with("/attendance_group", function () use ($KLEIN, $APPLICATION) {
                    ...
                    $KLEIN->respond("POST", "/removeRecords", function () use ($APPLICATION) {
                        return json_encode($APPLICATION->FUNCTIONS->ATTENDANCE_GROUP_CONTROL->archiveRecordsWithLog(json_decode($_POST["data"], true)));
                    });
                });
                ...
                $KLEIN->with("/database_backups", function () use ($KLEIN, $APPLICATION) {
                    $KLEIN->respond("POST", "/addRecord", function () use ($APPLICATION) {
                        return json_encode($APPLICATION->FUNCTIONS->DATABASE_BACKUPS_CONTROL->add(json_decode($_POST["data"], true)));
                    });

                    $KLEIN->respond("POST", "/editRecord", function () use ($APPLICATION) {
                        return json_encode($APPLICATION->FUNCTIONS->DATABASE_BACKUPS_CONTROL->editRecordWithLog($_POST["id"], json_decode($_POST["data"], true)));
                    });

                    $KLEIN->respond("POST", "/removeRecords", function () use ($APPLICATION) {
                        return json_encode($APPLICATION->FUNCTIONS->DATABASE_BACKUPS_CONTROL->removeRecordsWithLog(json_decode($_POST["data"], true)));
                    });
                });

                $KLEIN->with("/loan_advance", function () use ($KLEIN, $APPLICATION) {
                    $KLEIN->respond("POST", "/addRecord", function () use ($APPLICATION) {
                        return json_encode($APPLICATION->FUNCTIONS->LOAN_ADVANCE_CONTROL->add(json_decode($_POST["data"], true), json_decode($_POST["statements"], true)));
                    });

                    $KLEIN->respond("POST", "/editRecord", function () use ($APPLICATION) {
                        return json_encode($APPLICATION->FUNCTIONS->LOAN_ADVANCE_CONTROL->editRecordWithLog($_POST["id"], json_decode($_POST["data"], true)));
                    });

                    $KLEIN->respond("POST", "/removeRecords", function () use ($APPLICATION) {
                        return json_encode($APPLICATION->FUNCTIONS->LOAN_ADVANCE_CONTROL->removeRecordsWithLog(json_decode($_POST["data"], true)));
                    });
                });
